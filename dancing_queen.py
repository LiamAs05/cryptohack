...
class ChaCha20:
    ...
    def _undo_block(self, state):
        self._undo_round(state, 3, 4, 9, 14)
        self._undo_round(state, 2, 7, 8, 13)
        self._undo_round(state, 1, 6, 11, 12)
        self._undo_round(state, 0, 5, 10, 15)
        self._undo_round(state, 3, 7, 11, 15)
        self._undo_round(state, 2, 6, 10, 14)
        self._undo_round(state, 1, 5, 9, 13)
        self._undo_round(state, 0, 4, 8, 12)

    def _undo_round(self, x, a, b, c, d):
...
if __name__ == '__main__':
    ...
    # Unlocking Keystream from PT-CT pair
    keystream = xor(msg, msg_enc)

    # Retrieving state from keystream
    state = bytes_to_words(keystream[:64])
    ...
    initial_state = words_to_bytes(state)
    key = initial_state[16:48]
    
    print(f"Key is: {key}")
    pt = c.decrypt(bytes.fromhex('b6327e9a2253034096344ad5694a2040b114753e24ea9c1af17c10263281fb0fe622b32732'), key, bytes.fromhex('a99f9a7d097daabd2aa2a235'))
